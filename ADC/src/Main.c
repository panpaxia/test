...
int main()
{
  ...
  for( i = 0; i < 20; i++ )
  {
    j = (3.3 * abcBuff[i]*100)/4096;
    PRINT( "abcBuff[%d] = %d U = %d\n", i, abcBuff[i], j);
  }

  /* 单通道采样：选择adc通道0做采样，对应 PA4引脚， 带数据校准功能 */
  PRINT( "\n2. PA4 Single channel sampling...\n" );
  ...
  {
    j = (3.3 * abcBuff[i]*100)/4096;
    PRINT( "abcBuff[%d] = %d, U = %d\n",i, abcBuff[i], j); // 注意：由于ADC内部偏差的存在，当采样电压在所选增益范围极限附近的时候，可能会出现数据溢出的现象
  }

  /* 差分通道采样：选择adc通道0做采样，对应 PA4(AIN0)、PA12(AIN2) */
  PRINT( "\n3. PA4(AIN0)、PA12(AIN2) Diff channel sampling...\n" );
  ...
  PRINT( "\n5. PA5 Single channel sampling in interrupt mode...\n" );
  GPIOA_ModeCfg( GPIO_Pin_5, GPIO_ModeIN_Floating );
  ADC_ExtSingleChSampInit( SampleFreq_3_2, ADC_PGA_0 );
  ADC_ChannelCfg( 1 );
  adclen = 0;
